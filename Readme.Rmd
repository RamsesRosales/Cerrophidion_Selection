...
The [first part](firts_part/Scripts_Paper.md) contains the shell code used to assembly of transcriptomes, annotation, expression analysis, phylogenetics, variant calling and selection analisys. There are several [scripts](scripts/Scripts_Description.md) used that can be found in the [scripts](scripts) folder. Some scripts are owned by a third party, in those cases we mentioned the location of the original script or the contact were the scripts might be found.

# second_part

The [second part](second_part/Cgodm_selection_BMC.md) contains the R scripts with the statistics analysis that compare selection signals in toxins vs nontoxins, the necessary data to run the analysis might be found withing the folder.
...
The [data](https://github.com/RamsesRosales/Cerrophidion_Selection/tree/main/data) directory contains the consensus transcriptomes used in the analysis.
